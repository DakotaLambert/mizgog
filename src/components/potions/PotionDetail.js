...
export const PotionDetail = () => {
	...
	useEffect(() => {
		getPotionById(potionId).then(setPotion);
	...
	return (
		<>
            ...
            alt="A potion"
				...
				<div className="potionIngredients">
					...
					<div className="test">
						{potion.potionIngredients.map((pIngredient) => (
							<IngredientDetail
								pIngredientId={pIngredient.id}
								key={pIngredient.id}
							/>
						))}
					</div>
					<button
						className="editPotionButton"
						onClick={() => {
				...
				<img className="potionDetailBookImageDiv" alt="A book behind potions" src={potionBackground} />
			</div>
